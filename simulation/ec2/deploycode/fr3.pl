...
$type = $ARGV[0];
#srand(100);
our %as, %leaves, %short, %rt, %tier, %parent, %shadow;
our $NUMLEG=1000;
our $NUMATT=1000;
our $MINCUST=100;
our $LEG=1000;
...
for $t (1, 2, 3)
{
	...
	while(1)
	{
		...
		for $victims (\%single, \%multi, \%all)
		{
			...
			elsif($type eq "nosig")
			{
				...
				while($start != $victim && $hops < 20)
				{
				    ...
				    if (exists($senss{$next}))
				    {
					$ltraffic{$start . "-" . $next} += $src{$lt};
				    }
				    $start = $next;
				}
			    }
			    ...
			    @sorted = sort {$candidates{$a}{'att'} <=> $candidates{$a}{'att'}} keys %candidates;
			    $cd = 0;
			    for $s (@sorted)
			    {
				$r =  $candidates{$s}{'leg'}/$ltraffic{'all'};
				$cdr = $cd/$ltraffic{'all'};
		...
		for $class (1,2,3)
		{
		    ...
		    $om = $oldmean{$class};
		    $abs = abs($mean-$om);
		    print "$class Mean $mean oldmean $om $abs\n";
		    if (abs($mean-$om) < $precision && $om >= 0)
		    {
			$stop++;
		    }
		    print "Results in $class " . scalar(@{$results{$class}}) . "\n";
		    if (scalar(@{$results{$class}}) >= 100)
		    {
			$enough++;
		    }
		    $oldmean{$class}=$mean;
