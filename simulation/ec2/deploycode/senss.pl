...
if ($#ARGV > 3)    
{
    if ($type =~ /sig/)
    {
	$alpha = int($ARGV[4])/100;
    }
    else
    {
	$cloudfile = $ARGV[4];
	$fh = new IO::File($cloudfile);
	while(<$fh>)
	{
	    $_ =~ s/\n//;
	    $_ =~ s/\s+//g;
...
for $t (1)
{
    ...
    our %senss, %single, %multi, %all, %rt, %src, %att, %original, %demote, %rtcand, %popular, %polluted, %opath, %npath;
    our $totalsenss;
    for $l (@limits)
    {
	print "Limit $l\n";
	...
	{
	    	%senss=();
		# select top or random or cloud points for deployment
		if ($selection eq "top")
		{
		    print "Selection top\n";
		    $i = 0;
		...
		for $victims (\%single, \%multi, \%all)
		{
			...
			if ($type eq "sig")
			{
				...
				if ($class == 1)
				{
				    print "$time class $class tier $t senss $totalsenss attack $attack dropped $dropped r $r\n";
				}
				elsif($class == 2)
				{
				    print "$time class $class tier $t senss $totalsenss attack $attack dropped $dropped r $r\n";
			...
			elsif($type eq "nosig")
			{
			    ...
			    if ($class == 1)
			    {
				print "$time class $class tier $t senss $totalsenss attack $attack dropped $dropped r $r cd $lr\n";
			    }
			    elsif($class == 2)
			    {
				print "$time class $class tier $t senss $totalsenss attack $attack dropped $dropped r $r cd $lr\n";
			...
			elsif($type eq "cross")
			{
			    ...
			    if ($class == 1)
			    {
				print "$time class $class tier $t senss $totalsenss fixed $r\n";
			    }
			    elsif($class == 2)
			    {
				print "$time class $class tier $t senss $totalsenss fixed $r\n";
