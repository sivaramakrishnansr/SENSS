						...
						foreach ($filter as $filter_dpid){
						   ...
						   if ($remove_action==True){
							...
							echo '</div>';
							   ...
							   echo '<tr><td>'.$dpid.'</td><td>'.$dpid_of_switch.'</td><td>'.$packet_diff.'</td><td>'.$byte_diff.'</td><td><button type="button" class="btn btn-danger" data-toggle="modal" data-target="#Add-'.$dpid.'">Add Filter</button></td></tr>';
						   }
