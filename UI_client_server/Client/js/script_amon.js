...
function populateMonitoringValues(rowId, as_name, data) {
	if (data.packet_count=="Not reachable"){
		...
		for (var key in sum_array[as_name]){
    		...
    		if (parseInt(as_speed) >= 30*1000*1000*1000) {
        		cy.$("#root_" + as_name).data("name", display_threshold(parseInt(as_speed))).style("line-color", "red");
    		} 

		if (parseInt(as_speed)>0 && parseInt(as_speed) < 30*1000*1000*1000){
		...
		for (var as_name in graph_elements){
			...
			for (var i=0;i<all_edges.length;i++){
				...
				if(parseInt(edge_speeds[as_name][min_edge+"_"+max_edge])>0 && parseInt(edge_speeds[as_name][min_edge+"_"+max_edge])< 30*1000*1000*1000){
				...
				if(parseInt(edge_speeds[as_name][min_edge+"_"+max_edge])>=30*1000*1000*1000){
