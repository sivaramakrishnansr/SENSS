...
if (isset($_GET["add_controller"])){
	...
	$conn1->commit();
        ...
        $sql = sprintf("INSERT INTO CONSTANTS (id,controller_url) VALUES (NULL,'%s')", $input['controller_url']);
        $conn1->query($sql);
        $conn1->commit();
        return;
}
...
if (isset($_GET["get_controller"])){
        require_once "db.php";
        $sql = sprintf("SELECT controller_url from CONSTANTS");
        $result=$conn1->query($sql);
	if ($result->num_rows > 0) {
		$controller_url=$result->fetch_assoc()["controller_url"];
		echo "Controller at ".$controller_url;
	}
	else{
