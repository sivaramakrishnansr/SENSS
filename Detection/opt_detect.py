...
def TCP_flood_filter(firststats,secondstats,suggestions):
  label='ho'
  values = []
...
def main():
  ...
  for d in first.dsts:
    ...
    for p in first.dsts[d]:
      ...
      if(d in (first.dsts and second.dsts) and p in (first.dsts[d] and second.dsts[d])):
        ...
        if second.dsts[d][p].complete < params['THRESH_COMPLETE']*first.dsts[d][p].complete:
          ...
          detected = 1
          attack_type = SLOLORIS
          ermsg += "Destination %s:%s has too long connections, average increased from %.2f to %.2f ms\n" % (d, p, orig, cur)
        elif cur > params['THRESH_DUR']*orig and len(secondstats['ho'][d][p])/second.dsts[d][p] > 0.1:
          detected = 1
          attack_type = SLOLORIS
  ...
  for i in range(0,3):
      if (first.bytes[i] > 0 and second.bytes[i] > 10*first.bytes[i]): # 2 times the base fs file protocol bytes
        detected = 1
        attack_type = FLOOD
        ...
        attack_type = FLOOD
        suggestions['proto ' + tlabel[i]] = Filter("0.0.0.0\t",0,tlabel[i], float(second.bytes[i])/total, float(first.bytes[i])/ototal, 'proto ' + tlabel[i])
      used = float(second.bytes[i])/1000000000 # GB
      desired = 0.8*params['CAPACITY']
